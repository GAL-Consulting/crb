...
function listarCarpetas(parentId = FOLDER_ID) {
  gapi.client.drive.files.list({
    ...
    const select = document.getElementById("selectCarpetas");
    lista.innerHTML = "";
    select.innerHTML = "";

    // Opción de volver a raíz
    const volverLi = document.createElement("li");
    volverLi.textContent = "⬅️ Volver a raíz";
    volverLi.onclick = () => navegarCarpeta(FOLDER_ID, "Raíz");
    lista.appendChild(volverLi);

    const volverOpt = document.createElement("option");
    volverOpt.textContent = "⬅️ Volver a raíz";
    volverOpt.value = FOLDER_ID;
    ...
    carpetas.forEach(c => {
      ...
      li.onclick = () => navegarCarpeta(c.id, c.name);
    ...
    // Mostrar correctamente según el ancho
    actualizarVisibilidadListas();

    // ✅ Vincular una sola vez el evento onchange
    select.onchange = () => {
      const selectedId = select.value;
      const selectedName = select.options[select.selectedIndex].text;
      navegarCarpeta(selectedId, selectedName);
    };
  });
}

// Nueva función para centralizar la navegación entre carpetas
function navegarCarpeta(id, nombre) {
  document.getElementById("carpetaSeleccionada").textContent = nombre;
  listarCarpetas(id);
  listarArchivos(id);
}
...
function listarArchivos(carpetaId) {
  gapi.client.drive.files.list({
    ...
    const select = document.getElementById("selectArchivos");
    lista.innerHTML = "";
    select.innerHTML = "";
...
// Mantiene el comportamiento responsive correcto
function actualizarVisibilidadListas() {
  const esMovil = window.innerWidth < 768;
  const listaCarpetas = document.getElementById("listaCarpetas");
  const selectCarpetas = document.getElementById("selectCarpetas");
  const listaDocs = document.getElementById("listaDocs");
  const selectArchivos = document.getElementById("selectArchivos");

  if (esMovil) {
    listaCarpetas.style.display = "none";
    selectCarpetas.style.display = "block";
    listaDocs.style.display = "none";
    selectArchivos.style.display = "block";
  } else {
    listaCarpetas.style.display = "block";
    selectCarpetas.style.display = "none";
    listaDocs.style.display = "block";
    selectArchivos.style.display = "none";
  }
}
